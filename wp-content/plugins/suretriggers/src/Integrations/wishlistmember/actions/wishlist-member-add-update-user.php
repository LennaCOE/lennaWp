...
class WishlistMemberAddOrUpdateUser extends AutomateAction {
	...
	public function _action_listener( $user_id, $automation_id, $fields, $selected_options ) {
		...
		if ( function_exists( 'wlmapi_get_member' ) ) {
			$memberinfo = wlmapi_get_member( $user_id );
		}
		$memberaddress       = $memberinfo['member'] ? $memberinfo['member'][0]['UserInfo']['wpm_useraddress'] : '';
		$usercontext         = WordPress::get_user_context( $user_id );
		$usercontext['user'] = $memberaddress;
		return array_merge(
			$usercontext,
			$context
		);
