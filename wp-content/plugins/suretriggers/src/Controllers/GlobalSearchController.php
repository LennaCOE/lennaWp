		...
		foreach ( $form_posts as $form_post ) {
			...
			foreach ( $blocks as $key => $block ) {
				if ( 'uagb/forms' === $block['blockName'] ) {
					...
					foreach ( $block['innerBlocks'] as $element ) {
						if ( is_array( $element ) && isset( $element['blockName'] ) && 'uagb/forms' === $element['blockName'] ) {
							$options[] = [
								'label' => $form_post['post_title'] . ' (Form ' . ( $i ) . ')',
								'value' => $element['attrs']['block_id'],
							];
							$i++;
						} elseif ( is_array( $element ) && isset( $element['innerBlocks'] ) ) {
							foreach ( $element['innerBlocks'] as $inner_block ) {
								if ( isset( $inner_block['blockName'] ) && 'uagb/forms' === $inner_block['blockName'] ) {
									$options[] = [
										'label' => $form_post['post_title'] . ' (Form ' . ( $i ) . ')',
										'value' => $inner_block['attrs']['block_id'],
									];
									$i++;
								}
							}
	...
	public function search_fluent_support_triggers_last_data( $data ) {
		...
		if ( in_array( $term, [ 'response_added_by_agent', 'response_added_by_customer' ], true ) ) {
			...
			if ( 'ticket_created' == $term ) {
				...
				$tickets        = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fs_tickets WHERE status = 'closed' AND closed_by IN %s ORDER BY id DESC LIMIT 1", $customers_type[0] ) );
				if ( ! empty( $tickets ) ) {
					$customers     = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fs_persons WHERE id = %d ORDER BY id DESC LIMIT 1", $tickets[0]->customer_id ) );
					$ticket_data   = $tickets[0];
					$customer_data = $customers[0];
				...
				$agents_type = $wpdb->get_results( "SELECT id FROM {$wpdb->prefix}fs_persons WHERE person_type = 'agent'" );
				$tickets     = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fs_tickets WHERE status = 'closed' AND closed_by IN %s ORDER BY id DESC LIMIT 1", $agents_type[0] ) );
				if ( ! empty( $tickets ) ) {
					$customers     = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}fs_persons WHERE id = %d ORDER BY id DESC LIMIT 1", $tickets[0]->customer_id ) );
					$ticket_data   = $tickets[0];
					$customer_data = $customers[0];
